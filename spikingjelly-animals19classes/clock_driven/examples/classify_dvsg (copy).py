...
def main():
    ...
    * :ref:`中文API <classify_dvsg.__init__-cn>`
    ...
    print("T: ", args.T)
    print("B: ", args.b)
    print("C: ", args.channels)
    print("OPT: ", args.opt)
    print("LR: ", args.lr)
    out_dir = os.path.join(args.out_dir, 'T_{args.T}_b_{args.b}_c_{args.channels}_{args.opt}_lr_{args.lr}_')
    ...
    wandb.init(project="SL-Animals SNN 19 clases")
    wandb.config = {
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for frame, label in train_data_loader:
            ...
            #print("LABEL: ", label)
            label_onehot = F.one_hot(label, 19).float()
            #print("LABEL ONE HOT: ", label_onehot)
            #print("LABEL ONE HOT: ", label_onehot.shape)

            if args.amp:
                with amp.autocast():
                    #print( "FRAME SHAPE: ", frame.shape)
                    out_fr = net(frame)
                    #print("OUTFR: ", out_fr.shape)
                    loss = F.mse_loss(out_fr, label_onehot)
                    #print("LABEL ONE HOT: ", label_onehot.shape)
                scaler.scale(loss).backward()
                scaler.step(optimizer)
                scaler.update()
            else: # --------------------- entramos por aqui
                out_fr = net(frame)
                #print("OUTFR ELSE: ", out_fr.shape)
                #print("LABEL ONE HOT: ", label_onehot.shape)
                loss = F.mse_loss(out_fr, label_onehot)
                loss.backward()
                optimizer.step()
