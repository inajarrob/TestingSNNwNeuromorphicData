...
class VotingLayer(nn.Module):
    def __init__(self, voter_num: int):
        super().__init__()
        #print("voter_num: ", voter_num)
        #print("nn shape: ", self)
        self.voting = nn.AvgPool1d(voter_num, voter_num, padding=2)
...
def main():
    ...
    * :ref:`中文API <classify_dvsg.__init__-cn>`
    ...
    test_set = DVS128Gesture("/home/isabel/Desktop/tfm/SNN/spikingjelly/inferencia",
     train=False, data_type='frame', split_by='number', frames_number=args.T)
    ...
    try:
        checkpoint = torch.load("checkpoint_max19.pth", map_location='cpu')
    ...
    '''
    wandb.init(project="SL-Animals-SNN-11Clases")
    wandb.config = {
    ...
    for epoch in range(1):
        ...
        with torch.no_grad():
            for frame, label in test_data_loader:
                ...
                out_fr = net(frame)
                print("RESULT MODEL: ", out_fr.argmax(1).item())
                functional.reset_net(net)
        final_time = final_time + (time.time() - start_time)
        print(args)
    print(f'test_loss={test_loss}, test_acc={test_acc}, total_time={time.time() - start_time}')
    print("-----------------------------------------------------------------------------------------------")
