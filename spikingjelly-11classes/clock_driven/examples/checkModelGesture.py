...
def main():
    ...
    * :ref:`中文API <classify_dvsg.__init__-cn>`
    ...
    try:
        checkpoint = torch.load("/home/isabel/Desktop/tfm/SNN/spikingjelly/checkpoint_max.pth", map_location='cpu')
    ...
    for epoch in range(2):
        ...
        with torch.no_grad():
            for frame, label in test_data_loader:
                ...
                out_fr = net(frame)
                print("RESPONSE MODEL: ", out_fr.argmax(1).item())
                functional.reset_net(net)
        final_time = final_time + (time.time() - start_time)
        print(args)
    print(f'epoch={epoch}, test_loss={test_loss}, test_acc={test_acc}, total_time={time.time() - start_time}')
    print("-----------------------------------------------------------------------------------------------")
